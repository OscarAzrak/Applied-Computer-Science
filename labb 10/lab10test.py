...
def calculateWeight(rutan):
    if rutan != None:
        if rutan.down != None:
            vikt = (rutan.num * calculateWeight(rutan.down))
        else:
            vikt = rutan.num * HashTable.get(rutan.atom).vikt
        return vikt + calculateWeight(rutan.next)
    return 0
...
def checkMol(q):
    try:
        #readFormel(q)
        rutan = readFormel(q) #istället för rad 35
...
def readFormel(q):
    while q.peek() != ".":
        #readMol(q)
        rutan = readMol(q) #istället för rad 42
    return rutan
...
def readMol(q):
    #readGroup(q)
    rutan = readGroup(q) #istället för rad 48
